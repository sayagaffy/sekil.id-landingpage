 ...
 * major pages (/jurusan/[slug]).
 */
export const majorPost = defineType({
  name: 'majorPost',
  title: 'Majors / Jurusan (Content)',
  type: 'document',
  icon: BookIcon,
  groups: [
  ...
  fields: [
    ...
    defineField({
      ...
      type: 'string',
      group: 'content',
      description: 'Slug jurusan, contoh: teknik-informatika, psikologi',
      validation: (Rule) => Rule.required(),
    }),
    defineField({
      name: 'description',
  ...
  preview: {
    ...
    prepare({ title, slug }) {
      return { title, subtitle: `/jurusan/${slug ?? ''}` }
    },
  },
})
